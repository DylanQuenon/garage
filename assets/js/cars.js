...
addImage.addEventListener('click', () => {
    // compter combien j'ai d form-group pour les indices ex: cars_image_0_url
    ...
    const carsImages = document.querySelector("#cars_images")
    // recup le prototype dans la div
    const prototype = carsImages.dataset.prototype.replace(/__name__/g, index) // drapeau g pour indiqur que l'on va le faire plusieurs fois 
    carsImages.insertAdjacentHTML('beforeend', prototype)
    widgetCounter.value = index + 1
    handleDeleteButtons() //  pour mettre à jour la table deletes
})
const updateCounter = () => {
    const count = document.querySelectorAll("#cars_images div.form-group").length
